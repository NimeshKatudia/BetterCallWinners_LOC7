...
const customLabels = {
  Briefcase: "Stocks & Bonds",
  Home: "Property",
  Gem: "Gold",
  Car: "Vehicles & Equipments",
  Target: "Others",
};
...
export const GoalsTab = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (isEditing) {
      setGoals(
        goals.map((goal) => (goal.id === selectedGoal ? newGoal : goal))
      );
    } else {
    ...
    >
      ...
      <div className="bg-gradient-to-br from-indigo-500 to-indigo-600 rounded-2xl p-8 text-white shadow-xl">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-3xl font-bold">Get Instant Loan Approval</h2>
            <p className="mt-2 text-indigo-100">
              Apply for flexible loans and manage your finances effortlessly.
            </p>
          </div>
          <motion.button
        ...
        <div className="grid grid-cols-3 gap-6 mt-8">
          <div className="bg-white/10 backdrop-blur-sm rounded-xl p-4">
            <p className="text-indigo-100">Total Sanctianed Loans</p>
            <p className="text-3xl font-bold mt-1">{goals.length}</p>
          </div>
          <div className="bg-white/10 backdrop-blur-sm rounded-xl p-4">
            <p className="text-indigo-100">Toltal Undergoing Loans</p>
            <p className="text-3xl font-bold mt-1">
              {
                goals.filter(
                  (g) =>
                    parseFloat(calculateProgress(g.current, g.target)) < 100
                ).length
              }
            </p>
          </div>
          <div className="bg-white/10 backdrop-blur-sm rounded-xl p-4">
            <p className="text-indigo-100">Total Loans Repayed</p>
            <p className="text-3xl font-bold mt-1">
              {
                goals.filter(
                  (g) =>
                ...
                <div className="flex justify-between items-center">
                  <div>
                    <h3 className="text-2xl font-bold text-gray-900 dark:text-white">
                      {isEditing ? "Edit Loan Details" : "Apply for new loan"}
                    </h3>
                    <p className="mt-2 text-gray-500 dark:text-gray-400">
                      {isEditing
                        ? "Update your Loan details below."
                        : "Define your new loan with the details below."}
                    </p>
                  </div>
                  <motion.button
                ...
                <div className="space-y-6">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder="e.g, Business Marketing Loan"
                      required
                    />
                  </div>
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                      Choose an Asset to keep Mortgage
                    </label>
